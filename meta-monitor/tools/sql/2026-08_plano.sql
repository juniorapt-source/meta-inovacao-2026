...
--   GRANT SELECT, INSERT, UPDATE ON public.meta_inovacao_plano_acoes TO anon, authenticated;
--   DROP POLICY IF EXISTS select_publico ON public.meta_inovacao_plano_acoes;
--   CREATE POLICY "cc_select_publico" ON public.meta_inovacao_plano_acoes FOR SELECT TO anon USING (true);
--
...
--    Segue tools/sql/PADRAO_TABELA.md: GRANT antes das policies (sem GRANT, RLS
--    nunca chega a ser avaliado — não basta a policy existir) e uma única
...
CREATE POLICY "cc_select_publico" ON public.meta_inovacao_plano_acoes
  FOR SELECT TO anon
  USING (true);
