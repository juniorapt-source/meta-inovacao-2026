...
| 3.13 | Carimbo de origem comum (`grupo_local`): local no `.csv` (coluna "Grupo") e no payload da RPC desde já; a gravação no banco depende da migração | `canva.html`, `js/db-canva.js`, `tools/sql/2026-08_canva_grupo_local.sql` (novo) | Sonnet / médio | ✅ feito e **rodado em produção (28/08/2026)** |
| 3.14 | **[humano]** Rodar `tools/sql/2026-08_canva_grupo_local.sql` no SQL Editor | — | José | ✅ RODADO em 28/08/2026 — conferido em produção: `coluna=1, funcao=1, indice=1`. **Antes de rodar, a migração foi ensaiada inteira contra um PostgreSQL 16 real** (não só lida): 8 checagens — a chave extra sendo ignorada pela função ANTIGA (o site já a mandava), a migração aplicando limpa, dois projetos com o mesmo carimbo a partir de uma digitação, `grupo_local` ausente e malformado gravando com NULO em vez de derrubar, idempotência na segunda execução, linhas antigas intactas, e `cc_canva_editar` (não tocada) preservando o carimbo. O diff da função contra a original foi conferido por comparação automática: exatamente três acréscimos, nenhuma regra de validação/teto/canonização alterada. |
...
| 3.16 | **[humano]** José testa a rodada 3 em produção (mobile e desktop) | — | José | ⏳ **último item aberto do item 3** — o resto está em produção |
